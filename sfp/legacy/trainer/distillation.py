...
from sfp.legacy.utils.dataset import ShardingLMDBDataset, cycle
from sfp.legacy.utils.dataset import TextDataset, TextFolderDataset
from sfp.legacy.utils.distributed import (
    EMA_FSDP,
    fsdp_wrap,
    fsdp_state_dict,
    launch_distributed_job,
)
from sfp.legacy.utils.misc import set_seed, merge_dict_list
import torch.distributed as dist
from omegaconf import OmegaConf
from sfp.legacy.model import CausVid, DMD, SiD
import torch
import wandb
import time
import os
...
class Trainer:
    def __init__(self, config):
        ...
        if self.config.training_target == "low_noise":
            ...
            self.high_noise_step_list = torch.tensor(
                config.high_noise_step_list, dtype=torch.long
            )
            ...
            if self.config.training_target == "low_noise":
                ...
                for index, current_timestep in enumerate(self.high_noise_step_list):
                    ...
                    if index != len(self.high_noise_step_list) - 1:
                        next_timestep_id = 1000 - self.high_noise_step_list[
                            index + 1
                        ] * torch.ones(
            ...
            if self.is_main_process:
                ...
                if TRAIN_GENERATOR:
                    wandb_loss_dict.update(
                        {
                            "generator_loss": generator_log_dict["generator_loss"]
                            .mean()
                            .item(),
