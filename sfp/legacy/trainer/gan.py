...
from sfp.legacy.model import GAN
import torch
import wandb
import time
import os
...
class Trainer:
    ...
    def fwdbwd_one_step(self, batch, train_generator):
        ...
        total_loss = (
            (gan_D_loss + 0.5 * (r1_loss + r2_loss)) * loss_ratio * self.loss_scale
        )
    ...
    def all_gather_dict(self, target_dict):
        for key, value in target_dict.items():
            gathered_value = torch.zeros(
                [self.world_size, *value.shape], dtype=value.dtype, device=self.device
            )
