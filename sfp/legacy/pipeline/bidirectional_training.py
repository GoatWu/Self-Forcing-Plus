...
from sfp.model.wan_wrapper import WanDiffusionWrapper
from sfp.legacy.utils.scheduler import SchedulerInterface
...
class BidirectionalTrainingPipeline(torch.nn.Module):
        ...
        if self.scheduler.shift > 1:
            self.timestep_bound = (
                ...
                * (self.timestep_bound / 1000)
                / (1 + (self.scheduler.shift - 1) * (self.timestep_bound / 1000))
                * 1000
            )
    ...
    def inference_with_trajectory(
        self, noise: torch.Tensor, y, **conditional_dict
    ) -> torch.Tensor:
        """
        ...
        for index, current_timestep in enumerate(self.denoising_step_list):
            ...
            if not exit_flag:
                with torch.no_grad():
                    ...
                    next_timestep_id = 1000 - self.denoising_step_list[
                        index + 1
                    ] * torch.ones(
